...
export function ProductPageClient({ initialData, productId }: ProductPageClientProps) {
  ...
  const getCurrentImages = (): string[] => {
    ...
    if (selectedVariant && selectedVariant.images.length > 0) {
      return selectedVariant.images.map(img => `${baseUrl}/${img.path}`);
    }
    
    return [
      `${baseUrl}/${data.product.main_image.path}`,
      ...data.product.images.map(img => `${baseUrl}/${img.path}`)
    ];
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="flex space-x-2 overflow-x-auto">
                {currentImages.map((image, index) => (
                  ...
                  >
                    <Image
                      src={image}
                      alt={`${data.product.name} widok ${index + 1}`}
                      width={64}
                      height={64}
          ...
          <div className="space-y-6">
                  ...
                  {data.variants.map((variant) => (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        {variant.color.image && (
                          <Image
                            src={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/${variant.color.image.path}`}
            ...
            {data.sizes.length > 0 && (
              <div>
                <h2 className="text-lg font-medium text-gray-900 mb-3">Rozmiar</h2>
                <div className="grid grid-cols-2 gap-3">
                  {data.sizes.map((size) => {
            ...
            {/* Quantity Selector */}
            <div>
              <h2 className="text-lg font-medium text-gray-900 mb-3">Ilość</h2>
              <div className="flex items-center space-x-3">
                <button
        ...
        {data.product.description && (
          <div className="mt-12 border-t pt-8">
            ...
            <div 
              className="prose-gray text-gray-700 leading-relaxed"
